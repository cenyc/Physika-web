...
function vtkVolumeController(publicAPI, model) {
  ...
  publicAPI.changeActor = (actor) => {
    ...
    //选择新的颜色表后，触发vtkPiecewiseGaussianWidget颜色表刷新
    ...
    model.el.innerHTML = `
      ...
      <div class="${style.line} js-toggle">
        <div class="${style.sliderEntry}">
          ...
          <input type="range" min="0" max="1" value="0.4" step="0.01" class="${style.slider
      } js-spacing" />
        </div>
        <div class="${style.sliderEntry}">
          <div class="${style.sliderIcon}">${svgEdge}</div>
          <input type="range" min="0" max="1" value="0.2" step="0.01" class="${style.slider
      } js-edge" />
        </div>
      </div>
